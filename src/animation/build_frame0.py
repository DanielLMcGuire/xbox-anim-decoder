...
pos_anims, rot_anims = compute_anim_state(0.0, quats)


def frame0_world_mat(matScale, matOffset, idxPosAnim, idxRotAnim):
...
for ptype in primitive_types(data, surfofrev_vers):
    verts, faces = build_instances(ptype, data, quats, frame0_world_mat)
    write_obj(f'{outdir}/{ptype["name"]}.obj', verts, faces,
              f"# {ptype['label']} first animation frame")
...
write_obj(f'{outdir}/all_primitives_combined.obj', combined_verts, combined_faces,
          "# All procedural primitives combined for first animation frame")
